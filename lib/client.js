...
module.exports = function (dependencies) {
  ...
  Client.prototype.write = function write (notification, device, count) {
    ...
    if (!this.session || this.session.closed || this.session.destroyed) {
      const session = this.session = http2.connect(`https://${this.config.address}`, this.config);
      ...
      const closeAndDestroy = (callback) => {
        if (this.session === session) {
          this.session = null;
        }
        if (!session.closed) {
          session.close(() => destroy(callback));
        } else {
          destroy(callback);
        }
      };
